Shader "Custom/Hologram"
{
    Properties
    {
        ...
        _RimIntensity ("Rim Intensity", Range(0,16)) = 0.5
        _RimPower ("Rim Power (curve)", Range(0,16)) = 2.0
        _RimThreshold ("Rim Threshold (min dot)", Range(0,1)) = 0.0

        _RimBlinkSpeed ("Rim Blink Speed", Range(0,10)) = 2.0
        _RimBlinkAmplitude ("Rim Blink Amplitude", Range(0,1)) = 0.8

        _HologramMinAlpha ("Hologram Min Alpha", Range(0,1)) = 0.0
    ...
    SubShader
    {
        ...
        float _RimBlinkSpeed;
        float _RimBlinkAmplitude;
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            float time = _Time.y;
            float blink = sin(time * _RimBlinkSpeed) * 0.5 + 0.5;
            float blinkScale = lerp(1.0 - _RimBlinkAmplitude, 1.0 + _RimBlinkAmplitude, blink);
            float effectiveRim = _RimIntensity * blinkScale;
            ...
            float3 rimContribution = rimFactor * effectiveRim * _RimColor.rgb;
            o.Emission = _EmissionColor.rgb * _EmissionIntensity + rimContribution;

            float rimAlpha = saturate(rimFactor * effectiveRim);
            o.Alpha = lerp(_HologramMinAlpha, 1.0, rimAlpha);
